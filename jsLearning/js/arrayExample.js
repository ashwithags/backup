...
function getDetail(){
	...
	xhttp.onreadystatechange = function(){
		if((this.readyState == 4)&&(this.status == 200)){
			var random = JSON.parse(this.responseText).results;
			console.log(typeof(this.responseText));
			console.log(typeof(random));
			for(var i=0;i<random.length;i++){
				tableData +='<tr><td><img src='+random[i].picture.medium+'></td><td style="text-transform: capitalize;">'+random[i].name.title+'. '+random[i].name.first+' ' +random[i].name.last+'</td><td style="text-transform: capitalize;">'+random[i].gender+'</td><td style="text-transform: capitalize;">'+random[i].location.street+'</br>'+random[i].location.city+'</br>'+random[i].location.state+'</br>'+ random[i].location.postcode+'</td> <td><a href=mailto:'+random[i].email+'>'+random[i].email+'</a></td><td>'+random[i].dob+'</td><td><a href=tel:'+random[i].phone+'>'+random[i].phone+'</a></td><td><a href=tel:'+random[i].cell+'>'+random[i].cell+'</a></td><td>'+random[i].id.name+random[i].id.value+'</td><td>'+random[i].nat+'</td><td><button onclick="deleteItem('+i+')">Delete My Data</button></td></tr>';
			}
			document.getElementById('profile').innerHTML=structure+tableData+'</tbody></table>';
		}
	};
	xhttp.open('GET','https://randomuser.me/api/?gender=female&results=10',true);
	xhttp.send();
}
